...
- Resolved that Vietnamese alone determines V1 launch readiness; English remains localization-ready but non-blocking. The discussion also defined explicit `/vi` and reserved `/en` routing, approval-gated language switching, localized slugs linked by stable internal keys, canonical/hreflang behavior, no cross-language fallback, and centralized translatable content without deciding Ticket 04's storage format.
