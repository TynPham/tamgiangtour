...
V1 is clear enough to implement as small vertical slices that produce a mobile-first, Vietnamese-first website capable of generating measurable contact leads and booking enquiries for the primary Tam Giang Lagoon tour. The current bilingual VI/EN direction remains in force until the bilingual-contract ticket resolves its V1 priority.
...
- V1's primary audience is Vietnamese travelers. Phone and Zalo are required launch channels; WhatsApp is optional and must not block launch. This audience decision does not resolve the separate VI/EN scope question.
...
- The repository is currently the stock single-route Next.js starter. It has no product UI, content model, database integration, analytics, domain glossary, or ADRs.
...
- Detailed behavior, edge cases, and acceptance examples for each feature remain fog until the cross-cutting content, language, booking, and journey decisions are resolved.
