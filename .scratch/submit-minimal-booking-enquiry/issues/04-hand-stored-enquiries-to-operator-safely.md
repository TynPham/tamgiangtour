...
- The trusted recovery action is now exposed as `pnpm notifications:retry:telegram -- <enquiry-id>`; it reuses stored notification state, skips delivered messages, retries only safe failures, and stops ambiguous outcomes for manual investigation.
